...
export const generateAiSummary = async (req, res) => {
  try {
    ...
    const contentPrompt = `Generate a plain text summary of the following blog post in ${contentLength} words or less:
  
  Title: ${title}
  Content: ${content}
  no need to generate a full blog post or summary with title, just the summary.
      `;
      ...
      messages: [
        {
          role: "system",
          content:
            "You are a professional content writer specializing in entrepreneurship, marketing, and community building. You write in an engaging, authoritative voice that provides real value to business professionals.",
        },
        {
          role: "user",
          content: contentPrompt,
