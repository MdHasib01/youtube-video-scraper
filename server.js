...
async function generateBlogImage(title, summary, videoId) {
  try {
    ...
    // gpt-image-1 returns base64 data by default (and may return a URL).
...
import { scrapeAllChannels } from "./src/controllers/youtube.controller.js";
// Post Routes
...
cron.schedule("0 1 * * *", async () => {
  try {
    ...
    // Run the legacy YouTube-API job, but isolate it so its failures can
    // never stop the RSS scraper below.
    try {
      await processChannels();
    } catch (err) {
      console.error("processChannels failed (non-fatal):", err.message);
    }
